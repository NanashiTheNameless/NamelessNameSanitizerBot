...
# Configuration script for NamelessNameSanitizerBot
...
# Function to validate Discord snowflake IDs (owner/application IDs)
validate_discord_id() {
	local id_name="$1"
	local id_value="$2"
	local id_desc="$3"
...
FALLBACK_MODE=$(prompt_with_default "Fallback mode (default/randomized/static)" "default")
...
OWNER_DESTRUCTIVE_COOLDOWN_SECONDS=$(prompt_with_default "Cooldown for dangerous owner commands (seconds)" "30")
...
echo "2. Start the services: docker compose up -d --build"
